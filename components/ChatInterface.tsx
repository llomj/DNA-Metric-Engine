      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 custom-scrollbar">
        {messages.map((msg: any, i: number) => (
          <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            {msg.detectedFallacies?.map((f: any, idx: number) => (
              <div key={idx} className="mt-2 w-full p-2 bg-red-950/20 border border-red-600/30 rounded text-[9px] text-red-400 font-mono">DETECTED: {f.name}</div>
            ))}
          </div>
        ))}
